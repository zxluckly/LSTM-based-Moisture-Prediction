...
本项目基于若依框架扩展了**土壤墒情预测系统**，集成LSTM深度学习模型，实现智能土壤墒情预测和监控。

### 🎯 系统特色
- **智能预测**: 基于LSTM深度学习模型的时间序列预测
- **实时监控**: 土壤温度、湿度、EC值、pH值实时监控
- **数据质量**: 完整的数据预处理和质量控制流程
- **可视化大屏**: 丰富的图表展示和监控大屏
- **微服务架构**: LSTM模型独立部署，支持服务降级
...
├── ruoyi-admin/                    # 管理后台
├── ruoyi-system/                   # 系统模块
│   └── src/main/java/com/ruoyi/soil/  # 土壤墒情模块
├── ruoyi-ui/                       # 前端项目
│   └── src/views/soil/             # 土壤墒情页面
├── python-lstm-service/            # LSTM微服务
├── sql/                           # 数据库脚本
│   ├── soil_moisture_system.sql   # 系统数据表
│   └── soil_menu_init.sql         # 菜单权限初始化
├── 快速部署指南.md                 # 部署指南
├── 土壤墒情预测系统说明.md         # 详细说明
...
#### 1. 环境准备
- Java 8+, MySQL 5.7+, Redis 3.0+, Node.js 12+
- Python 3.7+ (用于LSTM服务)

#### 2. 数据库初始化
```bash
# 导入基础数据
mysql -u root -p ry-vue < sql/ry_20231130.sql
mysql -u root -p ry-vue < sql/quartz.sql

# 导入土壤墒情系统数据
mysql -u root -p ry-vue < sql/soil_moisture_system.sql
```
...
### 📖 详细文档
- [快速部署指南](快速部署指南.md) - 详细的部署步骤
- [系统说明文档](土壤墒情预测系统说明.md) - 完整的系统介绍
